...
import { Package, Archive, Gift } from 'lucide-react';
...
function RewardGraphic({ reward, size = '100%' }) {
  const FrameComponent = reward?.type === 'frame' ? FRAME_COMPONENTS[reward.id] : null;
  if (!reward) return null;
  return (
    <div style={{ width: size, height: size, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      {FrameComponent ? (
        <div style={{ width: '90%', height: '90%', position: 'relative', zIndex: 2 }}>
          <FrameComponent />
        </div>
      ) : reward.type === 'glow' ? (
        ...
        reward.imageUrl ? (
          <img src={reward.imageUrl} alt={reward.name} style={{ width: '75%', height: '75%', objectFit: 'contain', zIndex: 2, position: 'relative', filter: 'drop-shadow(0 0 10px rgba(255,255,255,0.2))' }} />
        ) : (
...
export default function Rewards() {
  ...
  const showStudio = frameRewards.length > 0 && !loading && !error;
  ...
  const LOOT_CASES = [
    { id: 'bronze_case', name: 'Bronze Case', cost: 1000, icon: <img src="/lootboxes/bronze.png" alt="Bronze Case" style={{ width: 180, height: 180, objectFit: 'contain', mixBlendMode: 'screen', margin: '-20px 0', transform: 'scale(1.25)', WebkitMaskImage: 'radial-gradient(circle at center, black 45%, transparent 72%)', maskImage: 'radial-gradient(circle at center, black 45%, transparent 72%)' }} />, color: '#CD7F32' },
    { id: 'silver_case', name: 'Silver Case', cost: 2500, icon: <img src="/lootboxes/silver.png" alt="Silver Case" style={{ width: 180, height: 180, objectFit: 'contain', mixBlendMode: 'screen', margin: '-20px 0', transform: 'scale(1.25)', WebkitMaskImage: 'radial-gradient(circle at center, black 45%, transparent 72%)', maskImage: 'radial-gradient(circle at center, black 45%, transparent 72%)' }} />, color: '#C0C0C0' },
    { id: 'gold_case', name: 'Gold Case', cost: 5000, icon: <img src="/lootboxes/gold.png" alt="Gold Case" style={{ width: 180, height: 180, objectFit: 'contain', mixBlendMode: 'screen', margin: '-20px 0', transform: 'scale(1.25)', WebkitMaskImage: 'radial-gradient(circle at center, black 45%, transparent 72%)', maskImage: 'radial-gradient(circle at center, black 45%, transparent 72%)' }} />, color: '#FFD700' }
  ];
        ...
        <div className={styles.scrollerOverlay}>
          ...
          <div className={styles.scrollerWindow}>
            ...
            <motion.div 
              ...
              initial={{ x: -105 }}
              animate={{ x: isSpinning || wonReward ? -5355 : -105 }}
              transition={{ duration: isSpinning ? 4.5 : 0, ease: [0.05, 0.9, 0.1, 1] }}
            >
              {scrollerItems.map((item, idx) => {
                ...
                return (
                  <div key={idx} className={styles.scrollerItem} style={{ borderColor: cfg.color, boxShadow: `inset 0 0 30px ${cfg.color}40, 0 10px 20px rgba(0,0,0,0.5)` }}>
                    <div className={styles.scrollerItemIcon}>
                      <RewardGraphic reward={item} size="120px" />
                    </div>
          ...
          <AnimatePresence>
            {wonReward && (
              ...
              >
                ...
                >
                  <div style={{ width: '150px', height: '150px', margin: '0 auto 20px auto', filter: `drop-shadow(0 0 50px ${TIER_CONFIG[wonReward.tier].color})`, position: 'relative', zIndex: 2 }}>
                    <RewardGraphic reward={wonReward} />
                  </div>
